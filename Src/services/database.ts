...
function fromDb<T>(row: Record<string, unknown>): T {
  ...
  delete mapped.createdAt; // strip DB-only field
  return mapped as T;
}
...
async function bulkUpsert<T extends { id: string }>(table: TableName, rows: T[]): Promise<boolean> {
  if (!isSupabaseReady() || rows.length === 0) return false;
  const { error } = await supabase.from(table).upsert(rows.map(r => toDb(r)), { onConflict: 'id' });
...
// Map entity type strings to table names
